...
class _SingleBannerWidgetState extends State<SingleBannerWidget> {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<HomeController>(
        builder: (controller) => controller.singleBannerImageLoading ||
                controller.singleBannerList.isEmpty
            ? Container(
                ...
                decoration: BoxDecoration(
                    ...
                    image: const DecorationImage(
                        ...
                        fit: BoxFit.cover)),
                child: controller.singleBannerImageLoading
                    ? const Icon(
                        Icons.error_outline_outlined,
                        color: Colors.grey,
                      )
                    : Image.asset(MyImages.placeHolderImage),
              )
            : GestureDetector(
                onTap: () {
                  bool isFree = controller.singleBannerList[0].version == '0'
                      ? true
                      : false;
                  ...
                  if (controller.isGuest() && isFree == false) {
                    ...
                    Get.toNamed(RouteHelper.movieDetailsScreen,
                        arguments: [controller.singleBannerList[0].id, -1]);
                  }
                },
                child: Container(
                  width: double.infinity,
                  ...
                  child: ClipRRect(
                    ...
                    child: CustomNetworkImage(
                      boxFit: BoxFit.cover,
                      imageUrl:
                          '${UrlContainer.baseUrl}${controller.singleBannerImagePath}${controller.singleBannerList[0].image?.landscape}',
                    ),
                  ),
                ),
              ));
